    ...
    try:
        with Session(engine) as session:
            order = Order(
                ...
                shipping_fee=place_order.shipping_fee,
            )
            ...
            order_item = OrderItem(
                order_id=order_id,
                product_id=place_order.product_id,
                quantity=place_order.quantity,
                order_size=place_order.order_size,
            )

            session.add(order_item)

            # update the number of sold items
            current_sold = session.execute(
                select(Product.sold)
                .select_from(Product)
                .where(Product.product_id == place_order.product_id)
            ).all()

            updated_sold = current_sold[0][0] + 1
            session.execute(
                update(Product)
                .where(Product.product_id == place_order.product_id)
                .values(sold=updated_sold)
            )
...
async def get_to_pay_parcel(user_id: int):
    with Session(engine) as session:
        query = (
            ...
            .filter(Order.order_status == "pending", Order.user_id == user_id)
        )
    ...
    for col in result:
        temp = {
            ...
            "order_size": col[10],
            "total_price": calculate_total_price(col[3], col[9], col[8], col[10]),
            "order_id": col[11],
        }
...
async def get_to_ship_parcel(user_id: int):
    with Session(engine) as session:
        query = (
            ...
            .join(Seller, Product.seller_id == Seller.id)
            .join(User, Seller.user_id == User.id)
            .filter(Order.order_status == "confirmed", Order.user_id == user_id)
        )
...
async def get_to_deliver_parcel(user_id: int):
    with Session(engine) as session:
        query = (
            ...
            .join(Seller, Product.seller_id == Seller.id)
            .join(User, Seller.user_id == User.id)
            .filter(Order.order_status == "out_for_delivery", Order.user_id == user_id)
        )
...
async def get_to_review_parcel(user_id: int):
    with Session(engine) as session:
        query = (
            ...
            .join(Seller, Product.seller_id == Seller.id)
            .join(User, Seller.user_id == User.id)
            .filter(Order.order_status == "delivered", Order.user_id == user_id)
        )
