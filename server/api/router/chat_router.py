...
class ConnectionManager:
    ...
    async def store_message(self, chat: SendChat):
        ...
        if chat_doc:
            ...
            await self.chat.update_one({"_id": chat_doc["_id"]}, {"$set": {"messages": chat_doc["messages"]}})
        else:
            chat_doc = {
    ...
    async def chat_messages(self, user_id: int):
        ...
        chat_doc = await self.chat.find({'participants': user_id}, filter).to_list(length=None)
        ...
        for chat in chat_doc:
            recipient_id = chat.get('participants')[0] if chat.get('participants')[0] != user_id else chat.get('participants')[1]
